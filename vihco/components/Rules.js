  ...
  switch (gameName) {
    case 'Yatzi':
      gameRules = {
        ...
        rules: [
          "THE GAME:",
          " Upper section of the classic Yahtzee dice game. You have 5 dices and for the every dice you have 3 throws.",
          " After each throw you can keep dices in order to get same dice spot counts as many as possible. In the end of the turn you must select your points from 1 to 6. Game ends when all points have been selected. The order for selecting those is free.",
          " POINTS:",
          " After each turn game calculates the sum for the dices you selected. Only the dices having the same spot count are calculated. Inside the game you can not select same points from 1 to 6 again."
        ]
      };
      screenName = "Yatzi";
      break;
 ...
 <View style={[styles.container, {height: '100%', backgroundColor: 'black'}]}>
  ...
  <View style={{flexDirection: 'row'}}>
    <View style={[styles.flexLeft, {justifyContent: 'center',marginHorizontal:20}]}>
      <Text style={[styles.gameHeader]}>Rules of {gameRules.title} </Text>
      {gameRules.rules.map(rule => (
        <Text key={rule} style={[styles.gameHeader,{fontSize:17}]}>
          {rule}
        </Text>
      ))}
